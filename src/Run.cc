...
void Run::EndOfRun(G4bool print) 
{
    ...
    //run condition
    //
    G4Material* material = fDetector->GetDetMaterial();
    G4double density = material->GetDensity();

    G4String Particle = fParticle->GetParticleName();
    G4cout << "\n The run is " << numberOfEvent << " "<< Particle << " of "
         << G4BestUnit(fEkin,"Energy") << " through "
         << G4BestUnit(fDetector->GetDetSize(),"Length") << " of "
  ...
  //
    ...
    } else
    {
        for (it = fProcCounter.begin(); it != fProcCounter.end(); it++)
        {
            G4String procName = it->first;
            G4VProcess* process = processTable->FindProcess(procName, fParticle);
            G4double xs = store->GetCrossSectionPerVolume(fParticle,fEkin,process,material);
