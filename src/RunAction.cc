    ...
    fDetector(det), fPrimary(prim), fHistoManager(0)
{
    ...
    analysisManager->CreateNtuple("GEN", "part en pos dir");
    ...
    analysisManager->CreateNtuple("TRACK", "pos mom en");
    ...
    analysisManager->CreateNtupleDColumn("momentumX0");
    analysisManager->CreateNtupleDColumn("momentumY0");
    analysisManager->CreateNtupleDColumn("momentumZ0");
    analysisManager->CreateNtupleDColumn("energy0");
    ...
    //analysisManager->CreateNtupleDColumn("edepstep");
    analysisManager->CreateNtupleDColumn("t_i");
    analysisManager->CreateNtupleDColumn("t_f");
    analysisManager->FinishNtuple();

    analysisManager->CreateNtuple("ANALYSIS", "en");
    analysisManager->CreateNtupleSColumn("name");
    analysisManager->CreateNtupleDColumn("energy");
    //analysisManager->CreateNtupleDColumn("e_rec");
...
void RunAction::SelectOutputName(G4String name)
{
    outputName = name;
}
...
void RunAction::EndOfRunAction(const G4Run* aRun)
{
    ...
    G4Material* material = new G4Material("Magnesium", 12., 24.305*g/mole, 1.737*g/cm3); // fDetector->GetMaterial(); // FIX THIS OR REMOVE
    ...
    if (survive > 0)
    {
        G4cout << "(this makes little sense for now) \n\n Nb of incident particles surviving after "
